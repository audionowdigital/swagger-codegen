...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    private var offlineRequest = false

    /**
     May be overridden by a subclass if you want to control the session
     configuration.
     */
    open func createSessionManager(offlineRequest isOfflineRequest: Bool = false) -> Alamofire.SessionManager {
        let configuration = URLSessionConfiguration.default
        // take response from cache however old it may be
        if isOfflineRequest == true {
          configuration.requestCachePolicy = NSURLRequest.CachePolicy.returnCacheDataDontLoad
          self.offlineRequest = true
        }
    ...
    override open func execute(offlineRequest isOfflineRequest: Bool = false, _ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        let managerId:String = UUID().uuidString
        // Create a new manager for each request to customize its request header
        let manager = createSessionManager(offlineRequest: isOfflineRequest)
    ...
    private func processRequest(request: DataRequest, _ managerId: String, _ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        case is String.Type:
            validatedRequest.responseString(completionHandler: { (stringResponse) in
                ...
                if self.processResponse(response: stringResponse, offlineRequest: self.offlineRequest, completion: completion) { return }
        ...
        case is Void.Type:
            validatedRequest.responseData(completionHandler: { (voidResponse) in
                ...
                if self.processResponse(response: voidResponse, offlineRequest: self.offlineRequest, completion: completion) { return }
        ...
        case is Data.Type:
            validatedRequest.responseData(completionHandler: { (dataResponse) in
                ...
                if self.processResponse(response: dataResponse, offlineRequest: self.offlineRequest, completion: completion) { return }
        ...
        default:
            validatedRequest.responseJSON(options: .allowFragments) { response in
                ...
                if self.processResponse(response: response, offlineRequest: self.offlineRequest, completion: completion) { return }
